...
def send_chat_message(
    ...
    # 4. Retrieve RAG context (conversational exemplars + supportive knowledge notes).
    # This runs only after the crisis check above has already passed - RAG never
    # sees a message that safety_layer decided needs an immediate crisis response.
    # Any failure here (embedding API down, index missing, etc.) must not break
    # the chat - retrieval is an enhancement, not a hard dependency.
    try:
        retrieved_context = rag_retriever.retrieve(
            user_msg_text, category_hint=None
        )
    except Exception as e:
        logger.warning(f"CHAT API - RAG retrieval failed, continuing without it: {e}")
        retrieved_context = None

    # 5. Build rich context for Gemini
    gemini_context = context_inj.build_gemini_context(
        user_id, user_msg_text, detected_emotion, retrieved_context=retrieved_context
    )
    ...
    # 6. Query Gemini with enhanced context
    api_reply = query_gemini_api(system_instruction, contents_payload)
    
    # 7. Fallback with emotional intelligence
    if not api_reply:
