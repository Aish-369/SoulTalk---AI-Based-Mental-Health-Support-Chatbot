...
from .rag.prompt_context import format_retrieved_context
...
class ContextInjector:
    ...
    def build_gemini_context(self, user_id: int, user_message: str, 
                            ...
                            retrieved_context: Dict = None) -> Dict:
        """
        ...
        Args:
            ...
            retrieved_context: Optional RAG output, e.g.
                {"exemplars": [...], "knowledge": [...]} from rag.retriever.
                Pass None (or omit) when RAG retrieval was skipped or failed -
                the chatbot must work fine without it.
        ...
        # Format RAG output (if any) into prompt-ready text. Left as an empty
        # string when retrieval was skipped/failed so the prompt template
        # simply omits the section - RAG is additive, never required.
        retrieved_context_text = ""
        if retrieved_context:
            try:
                retrieved_context_text = format_retrieved_context(retrieved_context)
            except Exception as e:
                logger.warning(f"Failed to format retrieved RAG context, omitting it: {e}")
                retrieved_context_text = ""
...
GROUNDING RULES FOR RETRIEVED CONTEXT:
- Use the retrieved context above only where it genuinely fits what the user just said.
- Never invent facts, statistics, or claims and attribute them to this context.
- If the retrieved context doesn't actually contain enough to help, rely on your own
  supportive judgment instead of forcing a fit.
- Never mention "retrieved context", embeddings, vector search, or any other
  implementation detail to the user - just respond naturally.
"""
...
- You are a supportive companion, not a licensed therapist or doctor - never diagnose
  a condition, never make medical claims, and if the user needs professional care,
  gently encourage that alongside your support rather than instead of it
